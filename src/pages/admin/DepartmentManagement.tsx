...
import { addDepartment, updateDepartment, deleteDepartment, setDepartments, type Department } from '@/store/slices/departmentSlice';
...
export const DepartmentManagement = () => {
  ...
  const fetchDepartments = async () => {
    try {
      const response = await fetch('https://ems-api-data.onrender.com/departments');
...
const [currentPage, setCurrentPage] = useState(1);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const employeeCount = employees.filter(emp => emp.department === formData.name).length;
    if (editingDepartment) {
      // Update department via API
      try {
        const response = await fetch(`https://ems-api-data.onrender.com/departments/${editingDepartment.id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ ...editingDepartment, ...formData, employeeCount }),
        });
        const updated = await response.json();
        dispatch(updateDepartment(updated));
        fetchDepartments();
        toast({ title: 'Department Updated', description: `${formData.name} has been updated successfully` });
      } catch (error) {
      ...
      try {
        ...
        fetchDepartments();
        toast({ title: 'Department Added', description: `${formData.name} has been created successfully` });
      } catch (error) {
  ...
  const handleDelete = async (department: Department) => {
    try {
      await fetch(`https://ems-api-data.onrender.com/departments/${department.id}`, { method: 'DELETE' });
      dispatch(deleteDepartment(department.id));
      fetchDepartments();
      toast({ title: 'Department Deleted', description: `${department.name} has been removed from the system` });
    } catch (error) {
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                View and manage department information ({departments.length} total)
              </CardDescription>
            </div>
              ...
              <TableBody>
                {paginatedDepartments.map((department) => (
                  <TableRow key={department.id}>
                    ...
                    <TableCell>{employees.filter(emp => (emp.department?.trim().toLowerCase() === department.name.trim().toLowerCase())).length}</TableCell>
                    {/* <TableCell>${(department.budget / 1000).toFixed(0)}K</TableCell> */}
                    <TableCell>
